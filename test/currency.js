...
describe('Currency', function() {
  ...
  var helpthehomelessCentralData = {
    general: {
    ...
    exchange_rates: {
      helpthehomeless_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_helpthehomeless: 0.01388998
    }
  };

  it.skip('will make live request to helpthehomeless central', function(done) {
    var currency = new CurrencyController({});
    var req = {};
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.helpthehomeless_usd);
        (typeof response.data.helpthehomeless_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(helpthehomelessCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.helpthehomeless_usd);
        response.data.helpthehomeless_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
