...
[![Build Status](https://img.shields.io/travis/Altcoin-Cash/insight-api/master.svg?branch=master)](https://travis-ci.org/Altcoin-Cash/insight-api)
[![NPM version](https://img.shields.io/npm/v/@Altcoin-Cash/insight-api.svg)](https://npmjs.org/package/@Altcoin-Cash/insight-api)
...
> A Helpthehomeless blockchain REST and WebSocket API Service

This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/Altcoin-Cash/insight-ui.
...
```bash
npm install -g @Altcoin-Cash/helpthehomeless-node
helpthehomeless-node create mynode
cd mynode
helpthehomeless-node install @Altcoin-Cash/insight-api
helpthehomeless-node start  # to also start the service
```
...
- [Helpthehomeless Node Helpthehomeless 4.x](https://github.com/Altcoin-Cash/helpthehomeless-node)

**Note:** You can use an existing Helpthehomeless data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` need to be enabled in `helpthehomeless.conf`, as well as a few other additional fields.

### Query Rate Limit

To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `helpthehomeless-node.json` with:
...
With all the configuration options available: https://github.com/Altcoin-Cash/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
[
  {
    ...
    DataObject: {
      ...
      type: 1,
      url: 'https://www.helpthehomeless.org'
    },
    AbsoluteYesCount: 40,
    YesCount: 40,
    NoCount: 0,
...
```
{
  "result":"[[\"proposal\",{\"end_epoch\":1519848619,\"name\":\"ghijklmnopqrstuvwxyz01234567891519097947\",\"payment_address\":\"yik5HAgVAgjH1oZKjcDfvcf22bwBNbSYzB\",\"payment_amount\":10,\"start_epoch\":1519097947,\"type\":1,\"url\":\"https://www.helpthehomelesscentral.org/p/test_proposal_1519097947\"}]]",
  "error":null,
  "id":78637
}
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in helpthehomelessd.
- The endpoint for `/peer` is no longer relevant connection to helpthehomelessd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking helpthehomelessd.
...
- (Medium)[How to setup a Helpthehomeless Instant-Send Transaction using Insight API - The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)

## Contributing

Feel free to dive in! [Open an issue](https://github.com/Altcoin-Cash/insight-api/issues/new) or submit PRs.
